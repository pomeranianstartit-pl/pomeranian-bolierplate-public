...
std::vector<key_t_defined> AiSD::BST::DisplayKeys()
{
	std::vector<key_t_defined> keys;
	BSTNode* min = Min();
...
std::vector<key_t_defined> AiSD::BST::DisplayKeys(BSTNode* subtree_root)
{
	std::vector<key_t_defined> keys;
	BSTNode* min = Min(subtree_root);
	keys.push_back(min->key);
	key_t_defined max = Max(subtree_root)->key;
	while (min->key < max)
	{
		min = Succesor(min->key);
		keys.push_back(min->key);
...
void AiSD::BST::CreateFromData(std::vector<key_t_defined> keys, std::vector<std::string> names)
{
...
void AiSD::BST::Insert(const key_t_defined k, const std::string name)
{
...
AiSD::BSTNode* AiSD::BST::Search(const key_t_defined k)
{
	BSTNode* tmp = firstNode;
	while (tmp && k != tmp->key)
	{
...
AiSD::BSTNode* AiSD::BST::Succesor(const key_t_defined k)
{
...
{
	std::vector<key_t_defined> keys = DisplayKeys(subtree_root);
	return keys.size();
}