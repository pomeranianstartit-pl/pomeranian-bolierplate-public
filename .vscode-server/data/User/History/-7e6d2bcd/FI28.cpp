...
void AiSD::BST::generateNamesDotFile(BSTNode* root, std::string file_name) {
	...
	while (!q.empty()) {
		...
		q.pop();
		if (current->left != NULL) {
			file << current->data.name << " -> " << current->left->data.name << ";\n";
			q.push(current->left);
		}
		if (current->right != NULL) {
			file << current->data.name << " -> " << current->right->data.name << ";\n";
			q.push(current->right);
		}
	}
	file << "}";
...
std::vector<defined_key_t> AiSD::BST::DisplayKeys()
{
	std::vector<defined_key_t> keys;
	BSTNode* min = Min();
	keys.push_back(min->key);
	defined_key_t max = Max()->key;
	while (min->key < max)
	{
...
std::vector<defined_key_t> AiSD::BST::DisplayKeys(BSTNode* subtree_root)
{
	std::vector<defined_key_t> keys;
	BSTNode* min = Min(subtree_root);
	keys.push_back(min->key);
	defined_key_t max = Max(subtree_root)->key;
	while (min->key < max)
	{
		min = Succesor(min->key);
		keys.push_back(min->key);
...
void AiSD::BST::CreateFromData(std::vector<defined_key_t> keys, std::vector<std::string> names)
{
...
void AiSD::BST::Insert(const defined_key_t k, const std::string name)
{
...
AiSD::BSTNode* AiSD::BST::Search(const defined_key_t k)
{
	BSTNode* tmp = firstNode;
	while (tmp && k != tmp->key)
	{
...
AiSD::BSTNode* AiSD::BST::Succesor(const defined_key_t k)
{
...
{
	std::vector<defined_key_t> keys = DisplayKeys(subtree_root);
	return keys.size();
}