...
export const HitTheMoleGame = () => {
  ...
  const createMole = (newMoles, id) => {
    ...
    return {
      ...
      timeout: setTimeout(() => moveMole(newPosition), MOLES_SPEED[0]),
    };
  };

  const shuffleMoles = () => {
    const newMolesPositions = [];

    for (let i = 0; i < molesNo; i++) {
      newMolesPositions.push(createMole(newMolesPositions));
    }

    setMoles(newMolesPositions);
  };

  const moveMole = (position) => {
    setMolesTimeouts((prevTimeouts) => {
      clearTimeout(prevTimeouts[position]);

      return { ...prevTimeouts, [position]: null };
    });

    const newMolesPositions = moles.filter(
      (molePosition) => molePosition !== position
    );
  ...
  //   for (const position in molesPositions) {

  //     molesTimeouts();

  //     setTimeout(() => moveMole(position), MOLES_SPEED[0]);
